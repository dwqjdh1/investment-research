...
# 仅启用 text2vec（国产中文模型）；禁止降级到 ChromaDB DefaultEmbeddingFunction，
# 否则会触发下载非中文优化的 all-MiniLM-L6-v2（境外 S3，国内极慢）
...
def _create_embedding_fn():
    """仅返回 text2vec embedding；加载失败直接抛异常，由上层降级关闭 RAG。
    严禁回退到 DefaultEmbeddingFunction（会下载 all-MiniLM-L6-v2）。"""
    if EMBEDDING_TYPE != "text2vec":
        raise RuntimeError(
            f"不支持 RAG_EMBEDDING={EMBEDDING_TYPE}；项目仅启用 text2vec"
        )
    ...
    fn(["测试文本"])  # 验证可用
    print("[RAG] 使用 text2vec-base-chinese 模型（国产中文优化）")
    return fn
